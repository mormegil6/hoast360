...
class Xr extends Plugin {
    ...
    init() {
        ...
        if (window.__hoastDprCorrect) {
            // Full device ratio, on every device. A cap of 2 for touch screens
            // was added on 2026-08-28 while hunting a tab death believed to be
            // GPU or memory pressure; the cause turned out to be an infinite
            // loop in the audio scheduler, and the cap never made any
            // measurable difference to it. Sharpness on a 3x phone screen is a
            // real loss, so it goes back rather than staying on a justification
            // that evaporated. Re-cap here if a device is ever shown to need it.
